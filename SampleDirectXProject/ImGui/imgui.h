...
namespace ImGui
{
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to value for the payload type.
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // value if rectangle (of given size, starting from cursor position) is visible / not clipped.
    IMGUI_API bool          IsRectVisible(const ImVec2& rect_min, const ImVec2& rect_max);      // value if rectangle (in screen space) is visible / not clipped. to perform coarse clipping on user's side.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // Disable catching mouse, hovering value with pass through.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupId              = 1 << 7,   // For IsPopupOpen(): ignore the ImGuiID parameter and value for any popup.
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 8,   // For IsPopupOpen(): search/value at any level of the popup stack (default value in the current level)
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_AcceptBeforeDelivery         = 1 << 10,  // AcceptDragDropPayload() will returns true even before the mouse button is released. You can then call IsDelivery() to value if the payload needs to be delivered.
...
//  B) render a complex 3D id inside a UI element without an intermediate texture/render target, etc.
